...
use crate::host::Host;
use crate::ops;
use crate::store::{ContextRecord, Roots, Store, MAX_FILES};

/// A question is part of the planner prompt, so unlike corpus text it enters a
/// model context directly. Bound it independently of the much larger corpus limit.
const MAX_QUERY_CHARS: usize = 32_000;

/// Context names are display metadata repeated in lists and recursive prompts.
const MAX_INLINE_CONTEXT_NAME_CHARS: usize = 200;
...
pub struct QueryBody {
    /// An existing context to answer from. Provide exactly one of `context_id` or
    /// `documents`. Existing clients can keep sending `{context_id, query}`.
    #[serde(default)]
    pub context_id: Option<String>,
    /// Inline documents to persist as a new context before answering. This lets one
    /// capability call analyze generated history without a separate context-creation
    /// round trip. Mutually exclusive with `context_id`.
    #[serde(default)]
    #[schema(inline)]
    documents: Option<Vec<DocumentBody>>,
    /// Label for an inline context. Valid only with `documents`; defaults to
    /// "inline query".
    #[serde(default)]
    ...
    /// Structural requirements the final answer must satisfy. Omit to preserve the
    /// normal caller behavior; supplied citations are still checked as observed.
    #[serde(default)]
...
impl QueryBody {
    ...
    fn into_validated(self, base: Budget) -> Result<ValidatedQuery, String> {
        let budget = self.budget(base);
        let evidence_policy = self.evidence_policy.validate()?;
        ...
        if query_chars > MAX_QUERY_CHARS {
            return Err(format!(
                "query is {query_chars} characters, over the {MAX_QUERY_CHARS} ceiling"
            ));
        }

        let mut input_digest = None;
        let input = match (self.context_id, self.documents) {
            (Some(context_id), None) => {
                if self.context_name.is_some() {
                    return Err(
                        "context_name is valid only when inline documents are provided".to_owned(),
                    );
                }
                let context_id = context_id.trim();
                if context_id.is_empty() {
                    return Err("context_id is empty".to_owned());
                }
                QueryContext::Existing(context_id.to_owned())
            }
            (None, Some(documents)) => {
                if documents.is_empty() {
                    return Err("inline documents must not be empty".to_owned());
                }
                if documents.len() > MAX_FILES {
                    return Err(format!(
                        "inline query has {} documents, over the {MAX_FILES}-document ceiling",
                        documents.len()
                    ));
                }
                if documents
                    .iter()
                    .any(|document| document.source.trim().is_empty())
                {
                    return Err("every inline document needs a non-empty source".to_owned());
                }
                let name = self
                    .context_name
                    .unwrap_or_else(|| "inline query".to_owned());
                let name = name.trim();
                if name.is_empty() {
                    return Err("context_name is empty".to_owned());
                }
                let name_chars = name.chars().count();
                if name_chars > MAX_INLINE_CONTEXT_NAME_CHARS {
                    return Err(format!(
                        "context_name is {name_chars} characters, over the \
                         {MAX_INLINE_CONTEXT_NAME_CHARS} ceiling"
                    ));
                }
                input_digest = Some(inline_input_digest(&documents));
                QueryContext::Inline {
                    name: name.to_owned(),
                    documents: documents
                        .into_iter()
                        .map(|document| Document::new(document.source, document.text))
                        .collect(),
                }
            }
            (Some(_), Some(_)) | (None, None) => {
                return Err("provide exactly one of context_id or inline documents".to_owned());
            }
        };
...
/// Stable digest over the exact UTF-8 document bytes accepted at the inline
/// boundary. Research submits one canonical JSONL document, so keeping this digest
/// equal to its SHA-256 history digest makes the cross-app proof directly comparable.
fn inline_input_digest(documents: &[DocumentBody]) -> String {
    if documents.len() == 1 {
        let mut digest = Sha256::new();
        digest.update(documents[0].text.as_bytes());
        return format!("sha256:{:x}", digest.finalize());
    }
    let mut digest = Sha256::new();
    digest.update((documents.len() as u64).to_le_bytes());
    for document in documents {
        digest.update((document.source.len() as u64).to_le_bytes());
        digest.update(document.source.as_bytes());
        digest.update((document.text.len() as u64).to_le_bytes());
        digest.update(document.text.as_bytes());
    }
...
async fn post_query(State(ctx): State<Arc<Ctx>>, Json(body): Json<QueryBody>) -> Response {
    let request = match body.into_validated(ctx.budget) {
        Ok(request) => request,
        Err(message) => return err(StatusCode::BAD_REQUEST, message),
    };

    // Resolve existing ids before checking the host, preserving the old route's
    // useful 404 for stale clients. Inline contexts are created only after the host
    // is known to be usable, so a temporarily unavailable model callback cannot
    // litter the durable context list with requests that never ran.
    let existing: Option<Arc<ContextRecord>> = match &request.input {
        QueryContext::Existing(context_id) => match ctx.store.get(context_id) {
            Some(record) => Some(record),
            None => return err(StatusCode::NOT_FOUND, "no such context"),
        },
        QueryContext::Inline { .. } => None,
    };
    ...
    let rec = match request.input {
        QueryContext::Existing(_) => existing.expect("existing context was resolved above"),
        QueryContext::Inline { name, documents } => {
            match ctx.store.create(&name, documents, None, None) {
                Ok(record) => record,
    ...
    .await;
    run.input_digest = request.input_digest;
...
mod tests {
    ...
    fn document(source: &str, text: &str) -> DocumentBody {
        DocumentBody {
    ...
    #[test]
    fn an_existing_context_request_remains_valid() {
        ...
        .into_validated(Budget::default())
        .expect("the legacy request shape remains supported");

        assert!(matches!(
            request.input,
            QueryContext::Existing(ref id) if id == "context-1"
        ));
    }

    #[test]
    fn inline_documents_become_a_valid_context_input() {
        let request = QueryBody {
            context_id: None,
            documents: Some(vec![document("campaign:attempts", "one\ntwo")]),
            context_name: Some(" Campaign history ".into()),
            query: "propose the next variant".into(),
            max_steps: Some(6),
            max_model_calls: None,
            max_depth: None,
            wall_secs: None,
        ...
        .into_validated(Budget::default())
        .expect("inline documents are accepted");

        match request.input {
            QueryContext::Inline { name, documents } => {
                assert_eq!(name, "Campaign history");
                assert_eq!(documents.len(), 1);
                assert_eq!(documents[0].source, "campaign:attempts");
            }
            QueryContext::Existing(_) => panic!("expected inline context"),
        }
    }

    #[test]
    fn inline_input_digest_uses_the_research_sha256_wire_format() {
        assert_eq!(
            inline_input_digest(&[document("campaign:c-1/history.jsonl", "history")]),
            "sha256:259aa8ef98a8b91de574cd904138ef643240c23080cf24da4793a6f10a43fa9d"
        );
    }

    #[test]
    fn query_requires_exactly_one_context_source() {
        for body in [
            QueryBody {
                context_id: None,
                documents: None,
            ...
            QueryBody {
                context_id: Some("existing".into()),
                documents: Some(vec![document("history", "attempt")]),
                context_name: None,
                query: "q".into(),
                max_steps: None,
                max_model_calls: None,
            ...
            let error = body
                .into_validated(Budget::default())
                .expect_err("ambiguous or absent context input must be refused");
            assert!(error.contains("exactly one"));
        }
    }

    #[test]
    fn inline_query_bounds_are_enforced_before_persistence() {
        let oversized_query = "q".repeat(MAX_QUERY_CHARS + 1);
        let error = QueryBody {
            context_id: None,
            documents: Some(vec![document("history", "attempt")]),
            context_name: Some("campaign".into()),
        ...
        .into_validated(Budget::default())
        .expect_err("oversized questions must be refused");
        ...
        .into_validated(Budget::default())
        .expect_err("empty document lists must be refused");
    ...
    fn a_documents_argument_spells_out_its_fields_instead_of_hiding_behind_a_ref() {
        ...
        let query = doc["components"]["schemas"]["QueryBody"].to_string();
        assert!(
            query.contains("documents") && query.contains("source") && query.contains("text"),
            "the query body hides the inline document fields: {query}"
        );
    }
