...
async fn main() -> anyhow::Result<()> {
    ...
    // `/openapi.json` rides INSIDE the same bearer gate as `/api/rlm/*`, at the SERVER
    // ROOT. Core fetches `http://127.0.0.1:<port>/openapi.json` on this sidecar's first
    // Healthy edge and lowers every operation it finds into searchable LLM tools, so
    // routing this one endpoint is what makes the whole `/api/rlm` surface callable by
    // an agent.
    //
    // Root, not under `/api/rlm`: Core tries the root FIRST, and keeping the document
    // off the mount keeps it out of the manifest's declared `http.routes[]` — anything
    // declared there is reachable through the generic ext-proxy, and the schema is
    ...
    // internal API surface to any other process on loopback.
    let protected = Router::new()
        .nest("/api/rlm", routes(ctx))
        .route(
            "/openapi.json",
            get(|| async { Json(ryu_rlm::api::openapi()) }),
        )
