    ...
    pub fn resolve(&self, candidate: &Path) -> Result<PathBuf> {
        ...
        for root in &self.0 {
            ...
            if relative.components().any(|component| {
                ...
                bail!(
                    "{} is a hidden or sensitive path and cannot be loaded explicitly",
                    full.display()
                );
            }
            return Ok(full);
...
impl Store {
    ...
    pub fn create(
        ...
        self.write_json(
            &self.dir.join("contexts").join(format!("{}.json", rec.id)),
            &rec,
        )?;
        let arc = Arc::new(rec);
        self.contexts
...
mod tests {
    ...
    fn a_path_outside_every_root_is_refused() {
        ...
        assert!(err
            .to_string()
            .contains("outside every configured context root"));
    }
    ...
    #[test]
    fn an_explicitly_named_hidden_file_is_refused() {
        // Explicit path selection must not turn the hidden-entry rule into a
        // credential exfiltration bypass.
        let root = tmpdir("explicit");
        std::fs::create_dir_all(root.join(".config")).unwrap();
        let f = root.join(".config").join("app.toml");
        std::fs::write(&f, "key = 1").unwrap();
        let roots = Roots(vec![root.canonicalize().unwrap()]);
        let (docs, skipped) = roots.load(&[f.display().to_string()]).unwrap();
        assert!(docs.is_empty());
        assert!(skipped.iter().any(|entry| entry.contains("hidden")));
    }
    ...
    fn the_outline_grows_with_chunk_count_not_corpus_size() {
        ...
        let ctx = store
            .create("doc", vec![doc("d.md", &body)], None, None)
            .unwrap();
