...
async fn map_chunks(
    ...
    for id in chunks {
        let Some(chunk) = ctx.chunk(*id) else { continue };
        let host = host.clone();
...
mod tests {
    ...
    fn older_observations_collapse_so_the_prompt_cannot_grow_without_limit() {
        ...
        for i in 0..20 {
            work.recent
                .push((format!("grep{i}"), format!("line one of {i}\n{}", "x".repeat(3_000))));
        }
        ...
        assert!(rendered.contains("grep19"), "the most recent step stays in full");
        assert!(rendered.contains("grep0"), "older steps stay listed, one line each");
    }
