...
//! tools appear as `rlm__ask`, `rlm__query`, … — which is exactly the
...
//! name, so a self-prefixed `rlm_ask` would register as `rlm__rlm_ask`.
//!
    ...
    match name {
        ...
        "query" | "ask" => {
            ...
            let record = if name == "ask" {
                let want = paths();
                if want.is_empty() {
                    return Err(anyhow!("`ask` needs `paths`; use `query` for a loaded context"));
                }
                let (docs, skipped) = ctx.roots.load(&want)?;
                if docs.is_empty() {
                    return Err(anyhow!(
            ...
            let run = engine::run(record.clone(), query, host, budget, 0).await;
...
mod tests {
    ...
    fn tool_names_are_bare_so_core_can_prefix_them() {
        // Core forms `{server}__{tool}`. A self-prefixed name becomes `rlm__rlm_ask`.
        for tool in tool_list().as_array().unwrap() {
            let name = tool["name"].as_str().unwrap();
            assert!(
                !name.starts_with("rlm"),
                "{name} is self-prefixed and would register as rlm__{name}"
            );
        }
    }
    ...
    fn the_deterministic_tools_are_advertised_as_needing_no_model() {
        ...
        let grep = tools
            ...
            .find(|t| t["name"] == "grep")
            .unwrap();
        assert!(grep["description"].as_str().unwrap().contains("NO model call"));
    }
}
