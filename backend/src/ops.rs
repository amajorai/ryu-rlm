    ...
    pub fn validate(&self, ctx: &ContextRecord) -> Result<()> {
        ...
        let check = |ids: &[usize]| -> Result<()> {
            ...
            if let Some(bad) = ids.iter().find(|i| **i >= n) {
                bail!("chunk {bad} does not exist (this context has {n}, numbered 0..{})", n - 1);
            }
            Ok(())
        };
        match self {
...
pub fn peek(ctx: &ContextRecord, id: usize, start: usize, len: Option<usize>) -> Observation {
    ...
    let take = len.unwrap_or(MAX_OBSERVATION_CHARS).min(MAX_OBSERVATION_CHARS);
...
pub fn grep(
    ...
    Observation::new(format!(
        "{hits} hit(s) in chunks {chunks_hit:?}\n{out}"
    ))
}
...
mod tests {
    ...
    fn ctx_of(docs: Vec<Document>) -> std::sync::Arc<ContextRecord> {
        let dir: PathBuf = std::env::temp_dir().join(format!("ryu-rlm-ops-{}", uuid::Uuid::new_v4()));
    ...
    fn map_refuses_a_fan_out_that_is_really_a_scan() {
        ...
        assert!(all.len() > MAX_MAP_FANOUT, "fixture must exceed the ceiling");
        let err = Op::Map {
            prompt: "q".into(),
            chunks: all,
        }
    ...
    fn every_op_round_trips_through_its_tagged_form() {
        for op in [
            ...
            Op::Peek { chunk: 1, start: 0, len: None },
            Op::Grep { pattern: "x".into(), max: None, context: None },
            Op::Map { prompt: "p".into(), chunks: vec![0] },
            Op::Recurse { query: "q".into(), chunks: vec![0] },
            Op::Note { text: "n".into() },
            Op::Final { answer: "a".into(), cites: vec![] },
        ] {
            let json = serde_json::to_value(&op).unwrap();
            assert_eq!(parse_op(&json).unwrap(), op, "round trip failed for {}", op.label());
        }
    }
}
