...
Recursive Language Models for Ryu — answer questions about a corpus far larger than any context window WITHOUT putting it in a prompt: the documents are held as a variable, a planner works over an outline of them, a cheap model reads one chunk at a time in parallel, and the findings fold back up with path:line citations and a replayable trace.
...
- Binary: `ryu-rlm` from the [Ryu releases](https://github.com/amajorai/ryu/releases).
- Crate: `cargo install ryu-rlm`.
...
# Recursive Language Model (`@ryu/rlm`)

Ask questions of a corpus far larger than any model's context window, without
